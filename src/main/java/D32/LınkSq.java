...
--SORU-6: arac tablosundaki en yüksek fiyat'ı listele
    ...
    SELECT MAX(fiyat) AS en_yuksek_fiyat --as takma ısım mıyudı ?
    FROM arac;


--Soru-7:arac tablosundaki araçların toplam fiyatını bulunuz

    SELECT SUM(fiyat) AS toplam_fiyat FROM arac;  --AS siz de dene

--Soru-8: arac tablosundaki fiyat ortalamalarını bulunuz
    SELECT AVG(fiyat)  ortalama_fiyat FROM arac;

    select Round(avg(fiyat),2) as Ortalama_Fiyat from arac; --sadece 2 haneli sonuc

--Soru-9: arac tablosunda kaç tane araç olduğunu bulunuz
    SELECT COUNT(*)  toplam_arac_sayisi FROM arac;  -- yıldız yerine id de olur. * daha garanti kullanım
...
--Soru-12: meslekler tablosunda ismi Ali olanların maaşını büyükten küçüğe doğru sıralayınız
    ...
    SELECT maas,isim FROM meslekler
    WHERE isim = 'Ali'
    ORDER BY maas DESC;
